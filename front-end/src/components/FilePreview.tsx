...
const FilePreview: React.FC<iFilePreviewProps> = (props) => {
  const { file, onClose, handleSendMessage, message, setMessage } = props;
    ...
    <dialog id="file-preview-modal" className="modal modal-bottom md:modal-middle justify-center">
      <div
        ...
        {file && <form className="w-full mt-4" onSubmit={handleSendMessage}>
          <input type="text" className="grow w-full text-lg input-md" placeholder="Message..." value={message} onChange={(e) => setMessage(e.target.value)} />
          <button className="btn btn-circle btn-primary mt-2" type="submit">
